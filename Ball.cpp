...
void Ball::addToScene()
  {
    ...
    Entity *entityPtr = sceneManagerPtr->createEntity("Ball", "sphere.mesh");
    entityPtr->setMaterialName("ball"); // set material for the Ball
    nodePtr = sceneManagerPtr->getRootSceneNode()-> createChildSceneNode("Ball"); // create a SceneNode
...
void Ball::moveBall(Real time)
  {
    ...
    Vector3 topPosition = sceneManagerPtr->getSceneNode("WallTop")->getPosition();
    Vector3 bottomPosition = sceneManagerPtr->getSceneNode("WallBottom")->getPosition();
    Vector3 leftPosition = sceneManagerPtr->getSceneNode("WallLeft")->getPosition();
    Vector3 rightPosition = sceneManagerPtr->getSceneNode("WallRight")->getPosition();
    ...
    // check if the Ball hit the bottom wall
    else if ((position.y - RADIUS) <= bottomPosition.y + (WALL_WIDTH / 2) && direction.y < 0) 
       {
          // place the Ball on the bottom wall
       	  nodePtr->setPosition(position.x, (bottomPosition.y + (WALL_WIDTH / 2) + RADIUS), position.z);
    ...
    // check if the Ball hit the top wall
    else if ((position.y + RADIUS) >= topPosition.y - (WALL_WIDTH / 2) && direction.y > 0)
       {
          ...
          nodePtr->setPosition(position.x, (topPosition.y - (WALL_WIDTH / 2) - RADIUS), position.z);
...
void Ball::hitPaddle()
  {
     ...
     Vector3 leftPaddlePosition = sceneManagerPtr->getSceneNode("LeftPaddle")->getPosition(); // left Paddle
     Vector3 rightPaddlePosition = sceneManagerPtr->getSceneNode("RightPaddle")->getPosition(); // right Paddle
    ...
    if ((ballPosition.x - RADIUS) < (leftPaddlePosition.x + (PADDLE_WIDTH / 2)))
     {
        // check for collision with left Paddle
        if ((ballPosition.y - RADIUS) < (leftPaddlePosition.y + (PADDLE_HEIGHT / 2)) && (ballPosition.y + RADIUS) > (leftPaddlePosition.y - (PADDLE_HEIGHT / 2)))
         {
           reverseHorizontalDirection(); // reverse the Ball's direction
           // place the Ball at the edge of the Paddle
           nodePtr->setPosition((leftPaddlePosition.x + (PADDLE_WIDTH / 2) + RADIUS), ballPosition.y, ballPosition.z);
         } // end if
      } // end if
 
    // is the Ball in range of the right Paddle?
    else if ((ballPosition.x + RADIUS) > (rightPaddlePosition.x - (PADDLE_WIDTH / 2)))
    {
    	// check for collision with right Paddle
    	if ((ballPosition.y - RADIUS) < (rightPaddlePosition.y + (PADDLE_HEIGHT / 2)) && (ballPosition.y + RADIUS) > (rightPaddlePosition.y - (PADDLE_HEIGHT / 2)))
          {
             reverseHorizontalDirection(); // reverse the Ball's direction
             // place the Ball at the edge of the Paddle
             nodePtr->setPosition((rightPaddlePosition.x - (PADDLE_WIDTH / 2) - RADIUS), ballPosition.y, ballPosition.z);
          } // end if
    }
  }